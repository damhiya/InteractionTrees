...
Section eqit.
  ...
  Inductive eqitP' (b1 b2: bool) : itree' E R1 -> itree' E R2 -> forall P : Type, (P -> itree E R1) -> (P -> itree E R2) -> Prop :=
  | EqRet r1 r2 k1 k2
          (REL : RR r1 r2) :
      eqitP' b1 b2 (RetF r1) (RetF r2) void k1 k2
  | EqTau t1 t2:
      eqitP' b1 b2 (TauF t1) (TauF t2) unit (const t1) (const t2)
  | EqVis {X} (e : E X) k1 k2 :
      eqitP' b1 b2 (VisF e k1) (VisF e k2) X k1 k2
  | EqTauL t1 ot2 P k1 k2
           (CHECK : b1)
           (REL : eqitP' b1 b2 (observe t1) ot2 P k1 k2) :
      eqitP' b1 b2 (TauF t1) ot2 P k1 k2
  | EqTauR ot1 t2 P k1 k2
           (CHECK : b2)
           (REL : eqitP' b1 b2 ot1 (observe t2) P k1 k2) :
      eqitP' b1 b2 ot1 (TauF t2) P k1 k2
  .
  Hint Constructors eqitP': core.

  Definition eqitP b1 b2 t1 t2 := eqitP' b1 b2 (observe t1) (observe t2).
  Hint Unfold eqitP: core.
  
  Definition eqitF b1 b2 := simulationF (eqitP b1 b2).
  Hint Unfold eqitF: core.
  
  Definition eqit b1 b2 := paco2 (simulationF (eqitP b1 b2) id) bot2.

  Lemma sim_mono b1 b2 vclo (MON: monotone2 vclo)
    : monotone2 (eqitF b1 b2 vclo).
  Proof.
    unfold eqitF, eqitP.
    unfold monotone2; intros; induction IN.
    remember (observe t1) as ot1 eqn:ET1.
    remember (observe t2) as ot2 eqn:ET2.
    destruct REL; econstructor; eauto; rewrite <- ET1; rewrite <- ET2; econstructor; eauto.
  Qed.
  Hint Resolve sim_mono : paco.
...
Ltac unfold_eqitP :=
  (try match goal with [|- eqitP _ _ _ _ _ _ _ ] => red end);
  (repeat match goal with [H: eqitP _ _ _ _ _ _ _ |- _ ] => red in H end).
...
Proof.
  unfold eqitF, eqitP.
  intros. destruct PR. econstructor; eauto. induction REL; eauto.
Qed.
