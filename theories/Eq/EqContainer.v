...
Section eqit.
  ...
  Inductive eqitR' (b1 b2: bool) : itree' E R1 -> itree' E R2 ->
    forall s : Shape itree_container, (Position itree_container s -> itree E R1) -> (Position itree_container s -> itree E R2) -> Prop :=
  | EqRet r1 r2
          (REL : RR r1 r2) :
      eqitR' b1 b2 (RetF r1) (RetF r2) RetShape void_elim void_elim
  | EqTau t1 t2:
      eqitR' b1 b2 (TauF t1) (TauF t2) TauShape (const t1) (const t2)
  | EqVis {X} (e : E X) k1 k2 :
      eqitR' b1 b2 (VisF e k1) (VisF e k2) (VisShape X) k1 k2
  | EqTauL t1 ot2 s k1 k2
           (CHECK : b1)
           (REL : eqitR' b1 b2 (observe t1) ot2 s k1 k2) :
      eqitR' b1 b2 (TauF t1) ot2 s k1 k2
  | EqTauR ot1 t2 s k1 k2
           (CHECK : b2)
           (REL : eqitR' b1 b2 ot1 (observe t2) s k1 k2) :
      eqitR' b1 b2 ot1 (TauF t2) s k1 k2
  .
...
Section Homogeneous.
  ...
  Lemma eqitR_trans (TRANS : Transitive RR) :
    forall x y z s x' y' z',
      @eqitR E R R RR false false x y s x' y' ->
      eqitR RR false false y z s y' z' ->
      eqitR RR false false x z s x' z'.
  Proof.
    unfold eqitR.
    intros x y z P kx ky kz H1 H2.
    remember (observe y) as oy1 in H1.
    remember (observe y) as oy2.
    
    destruct H1.
    - destruct H2; try congruence. inversion Heqoy1; subst. econstructor; eapply TRANS; eauto.
    - destruct H2; try congruence. inversion Heqoy1; subst. eauto.
    - dependent destruction H2; try congruence. rewrite <- x. eauto.
    - congruence.
    - congruence.
  Qed.
  ...
  Lemma eqitR_inv_RetF b1 b2 r ot P kx ky:
    @eqitR' E R R RR b1 b2 (RetF r) ot P kx ky -> eqitR' RR b1 b2 (RetF r) ot RetShape void_elim void_elim.
